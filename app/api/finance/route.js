...
export async function POST(req) {
    try {
        ...
        console.log("🚀 Final Anthropic API Request:", {
            ...
            messageStructure: JSON.stringify(anthropicMessages.map((msg) => ({
                role: msg.role,
...
Focus on clear financial insights and let the visualization enhance understanding.`,
        ...
        console.log("✅ Anthropic API Response received:", {
            ...
            toolOutput: response.content.find((c) => c.type === "tool_use")
                ? JSON.stringify(response.content.find((c) => c.type === "tool_use"), null, 2)
                : "No tool used",
        });
        const toolUseContent = response.content.find((c) => c.type === "tool_use");
        const textContent = response.content.find((c) => c.type === "text");
        const processToolResponse = (toolUseContent) => {
            if (!toolUseContent)
                return null;
            ...
            const processedChartConfig = Object.entries(chartData.chartConfig).reduce((acc, [key, config], index) => ({
                ...acc,
                [key]: {
                    ...config,
        ...
        console.error("Full error details:", {
            ...
            headers: error instanceof Error ? error.headers : undefined,
            response: error instanceof Error ? error.response : undefined,
        });
